...
def gen_obs(hr_obs, obs, ux, uy, blk_hr_grid_range, hr_shape, uav_num,
            ...
            p_msk, n_msk, mtx_size, charge_hr_grid_range, uav_e):
...
def test_subp(process_id,
    ...
    with torch.no_grad():
        ...
        sublog = SubLog(ENV_CONF, log_path=os.path.join(log_root_path, 'test'), process_id=process_id)
            ...
            while True:
                if not shared_ifdone.value:
                    ...
                    obs, uid_pos, p_msk, n_msk = gen_obs(hr_obs, obs, ux, uy, env.blk_hr_grid_range,
                                                         ...
                                                         st['cur_charge_station2hr_grid_range_channel'],
                                                         st['cur_uav_pos_energy_hit_mec_cec_charge'][
                                                         :, 2]
                                                         )
                    sublog.episode_info = []
